...
subcategory={
    ...
    init:function(){
        ...
        subcategory.delete();
        subcategory.kardex();
        subcategory.table('/inventory/setting/subcategory/active','');
    },
    new_subcategory:function(){
    ...
    edit:function(){
        $('#edit_item').on('click', function(event) {
            ...
            if(id.length>0){
                if(id.length<2){
                    var url     =   '/inventory/setting/subcategory/'+id+'/edit';
                    var method  =   'get';
                    var data    =   '';
                    ...
                    $form_validation_update.attr("action", '/inventory/setting/subcategory/'+result.id);
                }else{
                    $.gritter.add({
                        title: 'Notificacion',
                        text: 'Seleccione un subcategorye para editar',
        ...
        $('#delete_item').on('click', function(event) {
            ...
            if(id.length>0){
                bootbox.confirm("Are you sure?", function (result) {
                    $.gritter.removeAll();
                    if (result) {
                        var subcategory = id.toString();
                        var array = {id:subcategory}; 
                        
                        var url = '/inventory/setting/subcategory/all';
                        var method = 'delete';
                ...
                $.gritter.add({
                    title: 'Notificacion',
                    text: 'Porfavor selecione un subcategorye.',
                    image: 'http://a0.twimg.com/profile_images/59268975/jquery_avatar_bigger.png',
                    sticky: false,
                    time: '',
                    class_name: 'gritter-error'
    ...
    //muestra el kardex del subcategorye
    kardex:function(url){
        $('#kardex_subcategory').on('click', function(e) {
            var id = new Array();
            $("input:checkbox:checked").each(function(){
                if($(this).val()!='on' && $(this).val()!='true'){
                    id.push($(this).val());
            ...
            if(id.length>0){
                if(id.length<2){
                    window.location.href='/inventory/setting/subcategory/'+subcategory_id+'/kardex';
                }else{
                    $.gritter.add({
                        title: 'Notificacion',
                        text: 'Seleccione un subcategorye para editar',
